...
def mock_kuzu_connection() -> Generator[Mock, None, None]:
    with patch("kuzu.Connection") as mock_conn:
        ...
        mock_conn.return_value._get_node_table_names.return_value = [
            "Person",
            "Company",
        ]
        ...
        mock_conn.return_value.execute.return_value = Mock(
            has_next=lambda: False, get_column_names=lambda: ["column1"]
        )
...
def test_add_graph_documents_with_source(kuzu_graph: KuzuGraph) -> None:
    ...
    for query in expected_queries:
        assert any(
            query in call.args[0] for call in kuzu_graph.conn.execute.call_args_list
        )
...
def test_add_graph_documents_with_existing_source_id(kuzu_graph: KuzuGraph) -> None:
    ...
    assert any(
        "'existing_id'" in str(call) for call in kuzu_graph.conn.execute.call_args_list
    )
